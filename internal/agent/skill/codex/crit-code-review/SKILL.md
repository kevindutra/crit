...
Run this command (it blocks until the user finishes reviewing):
```bash
crit review --code --detach --wait
```

If the command exits with an error about tmux, ask the user to run the TUI manually:
...
After the review is complete, read the aggregate review comments:
